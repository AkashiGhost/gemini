...
(async () => {
  ...
  const screenshotPath = path.resolve('output/playwright/local-fake-mic.png');
  const logPath = path.resolve('output/playwright/local-fake-mic-log.txt');
  ...
  await context.grantPermissions(['microphone'], { origin: 'http://127.0.0.1:3012' });
  ...
  logs.push('[body] ' + bodyText.replace(/\s+/g, ' ').slice(0, 1200));
