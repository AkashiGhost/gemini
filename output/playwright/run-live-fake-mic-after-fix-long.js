...
(async () => {
  ...
  const screenshotPath = path.resolve('output/playwright/live-fake-mic-after-fix-long.png');
  const logPath = path.resolve('output/playwright/live-fake-mic-after-fix-long-log.txt');
  ...
  logs.push('[body] ' + bodyText.replace(/\s+/g, ' ').slice(0, 2500));
